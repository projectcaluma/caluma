...
class Answer(core_models.BaseModel):
    ...
    def modify_changed_choice_answer(self, question, answer_value, old_slug, new_slug):
    ...
    def copy(self, document_family=None, to_document=None, user=None):
        ...
        if self.question.type in [
            ...
            data_source = get_data_sources(dic=True)[self.question.data_source]
            ...
            for dynamic_option in DynamicOption.objects.filter(
                ...
                new_slug, new_label = data_source_object.on_copy(
                    old_answer=self,
                    new_answer=new_answer,
                    old_value=(dynamic_option.slug, dynamic_option.label),
                )

                # modify the answer value if the slug has changed
                if new_slug != dynamic_option.slug:
                    answer_value_changed = True
                    new_answer.value = self.modify_changed_choice_answer(
                        self.question, new_answer.value, dynamic_option.slug, new_slug
                    )

                # when the new slug has no value skip the creation of the new option
                if new_slug:
