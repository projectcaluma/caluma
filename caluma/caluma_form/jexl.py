...
class QuestionJexl(JEXL):
    ...
    def _get_referenced_fields(self, field: Field, dependencies: List[str]):
        referenced_fields = [
            ref_field
            for slug in dependencies
            for ref_field in self._structure.get_fields(slug)
        ]
    ...
    def is_hidden(self, field: Field):
        ...
        deps = list(self.extract_referenced_questions(field.question.is_hidden))
        referenced_fields = self._get_referenced_fields(field, deps)
